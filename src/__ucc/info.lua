...
local _info = {
    ...
    level = 'ok',
    synced = false,
    status = 'UniverseCreditcoin node down',
...
local function _exec_universecreditcoin_cli(...)
...
local function _get_universecreditcoin_cli_result(exitcode, stdout, stderr)
    if exitcode ~= 0 then
...
if _info.xsolidusd == 'running' then
    ...
    local _exitcode, _stdout, _stderr = _exec_universecreditcoin_cli('-datadir=data', 'getblockchaininfo')
    local _success, _output = _get_universecreditcoin_cli_result(_exitcode, _stdout, _stderr)
    ...
    local _exitcode, _stdout, _stderr = _exec_universecreditcoin_cli('-datadir=data', 'mnsync', 'status')
    local _success, _output = _get_universecreditcoin_cli_result(_exitcode, _stdout, _stderr)
